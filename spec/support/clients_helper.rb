module ClientsHelper
  def with_each_client
    ...
    oauth_client = JiraRestApi::Client.new({ :consumer_key => 'foo', :consumer_secret => 'bar' })
    oauth_client.set_access_token('abc', '123')
    clients["http://localhost:2990"] = oauth_client

    basic_client = JiraRestApi::Client.new({ :username => 'foo', :password => 'bar', :auth_type => :basic, :use_ssl => false })
