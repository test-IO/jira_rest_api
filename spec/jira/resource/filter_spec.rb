...
describe JiraRestApi::Resource::Filter do
  let(:client) do
    client = double()
    client.stub(:Issue) { JiraRestApi::Resource::IssueFactory.new(self) }
    client
  end
  ...
  let(:filter) do
    ...
    JiraRestApi::Resource::Filter.stub(:collection_path).and_return(collection_path)
    JiraRestApi::Resource::Filter.find(client, 42)
  end
  let(:jql_issue) do
    {
      :id => '663147',
      :self => 'https://localhost/rest/api/2/issue/663147',
      :key => "JiraRestApiRUBY-2386",
      :fields => {
        :reporter => jira_user,
