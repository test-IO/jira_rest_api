...
describe JiraRestApi::Base do

  class JiraRestApi::Resource::Deadbeef < JiraRestApi::Base # :nodoc:
  end

  class JiraRestApi::Resource::HasOneExample < JiraRestApi::Base # :nodoc:
    has_one :deadbeef
    has_one :muffin, :class => JiraRestApi::Resource::Deadbeef
    has_one :brunchmuffin, :class => JiraRestApi::Resource::Deadbeef,
                           :nested_under => 'nested'
    has_one :breakfastscone,
            :class => JiraRestApi::Resource::Deadbeef,
  ...
  class JiraRestApi::Resource::HasManyExample < JiraRestApi::Base # :nodoc:
    has_many :deadbeefs
    has_many :brunchmuffins, :class => JiraRestApi::Resource::Deadbeef,
                           :nested_under => 'nested'
  ...
  subject { JiraRestApi::Resource::Deadbeef.new(client, :attrs => attrs) }
  ...
  it "returns all the deadbeefs" do
    ...
    JiraRestApi::Resource::Deadbeef.should_receive(:collection_path).and_return('/jira/rest/api/2/deadbeef')
    deadbeefs = JiraRestApi::Resource::Deadbeef.all(client)
    deadbeefs.length.should == 1
    first = deadbeefs.first
    first.class.should == JiraRestApi::Resource::Deadbeef
  ...
  it "finds a deadbeef by id" do
    ...
    JiraRestApi::Resource::Deadbeef.should_receive(:collection_path).and_return('/jira/rest/api/2/deadbeef')
    deadbeef = JiraRestApi::Resource::Deadbeef.find(client, '98765')
  ...
  it "finds a deadbeef containing changelog by id" do
    ...
    deadbeef = JiraRestApi::Resource::Deadbeef.find(client, '98765', {expand:'changelog'})
    deadbeef.client.should == client
    deadbeef.attrs['self'].should  == 'http://deadbeef/'
    deadbeef.attrs['id'].should   == '98765'
    deadbeef.expanded?.should be_true
  ...
  it "builds a deadbeef" do
    deadbeef = JiraRestApi::Resource::Deadbeef.build(client, 'id' => "98765" )
  ...
  describe "fetch" do
    ...
    subject     { JiraRestApi::Resource::Deadbeef.new(client, :attrs => {'id' => '98765'}) }
  ...
  describe "save" do
    ...
    subject { JiraRestApi::Resource::Deadbeef.new(client) }
    ...
    it "returns false when an invalid field is set" do # The JiraRestApi REST API apparently ignores fields that you aren't allowed to set manually
      response.stub(:body => '{"errorMessages":["blah"]}', :status => 400)
      subject.stub(:new_record? => false)
      client.should_receive(:put).with('/foo/bar','{"invalid_field":"foobar"}').and_raise(JiraRestApi::HTTPError.new(response))
  ...
  describe "save!" do
    ...
    it "throws an exception when an invalid field is set" do
      ...
      end.should raise_error(JiraRestApi::HTTPError)
    end
  end
  ...
  describe 'url' do
    ...
    it "has a class method for the collection path" do
      JiraRestApi::Resource::Deadbeef.collection_path(client).should == "/foo/bar/deadbeef"
      #Should accept an optional prefix (flum in this case)
      JiraRestApi::Resource::Deadbeef.collection_path(client, '/flum/').should == "/foo/bar/flum/deadbeef"
    end

    it "has a class method for the singular path" do
      JiraRestApi::Resource::Deadbeef.singular_path(client, 'abc123').should == "/foo/bar/deadbeef/abc123"
      #Should accept an optional prefix (flum in this case)
      JiraRestApi::Resource::Deadbeef.singular_path(client, 'abc123', '/flum/').should == "/foo/bar/flum/deadbeef/abc123"
    end
  end
  ...
  it "returns the formatted attrs from to_s" do
    ...
    subject.to_s.should match(/#<JiraRestApi::Resource::Deadbeef:\d+ @attrs=#{Regexp.quote(attrs.inspect)}>/)
  end
  ...
  describe "extract attrs from response" do
    ...
    subject { JiraRestApi::Resource::Deadbeef.new(client, :attrs => {}) }
  ...
  describe "nesting" do
    ...
    it "defaults collection_attributes_are_nested to false" do
      JiraRestApi::Resource::Deadbeef.collection_attributes_are_nested.should be_false
    end

    it "allows collection_attributes_are_nested to be set" do
      JiraRestApi::Resource::Deadbeef.nested_collections true
      JiraRestApi::Resource::Deadbeef.collection_attributes_are_nested.should be_true
    end
  ...
  describe "has_many" do
    ...
    subject { JiraRestApi::Resource::HasManyExample.new(client, :attrs => {'deadbeefs' => [{'id' => '123'}]}) }

    it "returns a collection of instances for has_many relationships" do
      subject.deadbeefs.class.should == JiraRestApi::HasManyProxy
      subject.deadbeefs.length.should == 1
      subject.deadbeefs.each do |deadbeef|
        deadbeef.class.should == JiraRestApi::Resource::Deadbeef
      end
    end

    it "returns an empty collection for empty has_many relationships" do
      subject = JiraRestApi::Resource::HasManyExample.new(client)
      subject.deadbeefs.length.should == 0
    end

    it "allows the has_many attributes to be nested inside another attribute" do
      subject = JiraRestApi::Resource::HasManyExample.new(client, :attrs => {'nested' => {'brunchmuffins' => [{'id' => '123'},{'id' => '456'}]}})
      subject.brunchmuffins.length.should == 2
      subject.brunchmuffins.each do |brunchmuffin|
        brunchmuffin.class.should == JiraRestApi::Resource::Deadbeef
      end
    end

    it "allows it to be deeply nested" do
      subject = JiraRestApi::Resource::HasManyExample.new(client, :attrs => {'nested' => {
      ...
      subject.breakfastscones.each do |breakfastscone|
        breakfastscone.class.should == JiraRestApi::Resource::Deadbeef
      end
    end

    it "short circuits missing deeply nested attrs" do
      subject = JiraRestApi::Resource::HasManyExample.new(client, :attrs => {
                                                  'nested' => {}
      })
    ...
    it "allows the attribute key to be specified" do
      subject = JiraRestApi::Resource::HasManyExample.new(client, :attrs => {'irregularlyNamedThings' => [{'id' => '123'},{'id' => '456'}]})
      subject.irregularly_named_things.length.should == 2
      subject.irregularly_named_things.each do |thing|
        thing.class.should == JiraRestApi::Resource::Deadbeef
      end
    end
  ...
  describe "has_one" do
    ...
    subject { JiraRestApi::Resource::HasOneExample.new(client, :attrs => {'deadbeef' => {'id' => '123'}}) }

    it "returns an instance for a has one relationship" do
      subject.deadbeef.class.should == JiraRestApi::Resource::Deadbeef
      subject.deadbeef.id.should == '123'
    end

    it "returns nil when resource attribute is nonexistent" do
      subject = JiraRestApi::Resource::HasOneExample.new(client)
      subject.deadbeef.should be_nil
    end

    it "returns an instance with a different class name to the attribute name" do
      subject = JiraRestApi::Resource::HasOneExample.new(client, :attrs => {'muffin' => {'id' => '123'}})
      subject.muffin.class.should == JiraRestApi::Resource::Deadbeef
      subject.muffin.id.should == '123'
    end

    it "allows the has_one attributes to be nested inside another attribute" do
      subject = JiraRestApi::Resource::HasOneExample.new(client, :attrs => {'nested' => {'brunchmuffin' => {'id' => '123'}}})
      subject.brunchmuffin.class.should == JiraRestApi::Resource::Deadbeef
      subject.brunchmuffin.id.should == '123'
    end

    it "allows it to be deeply nested" do
      subject = JiraRestApi::Resource::HasOneExample.new(client, :attrs => {'nested' => {
        'breakfastscone' => { 'breakfastscone' => {'id' => '123'} }
      }})
      subject.breakfastscone.class.should == JiraRestApi::Resource::Deadbeef
      subject.breakfastscone.id.should == '123'
    end

    it "allows the attribute key to be specified" do
      subject = JiraRestApi::Resource::HasOneExample.new(client, :attrs => {'irregularlyNamedThing' => {'id' => '123'}})
      subject.irregularly_named_thing.class.should == JiraRestApi::Resource::Deadbeef
  ...
  describe "belongs_to" do
    ...
    class JiraRestApi::Resource::BelongsToExample < JiraRestApi::Base
      belongs_to :deadbeef
    end

    let(:deadbeef) { JiraRestApi::Resource::Deadbeef.new(client, :attrs => {'id' => "999"}) }

    subject { JiraRestApi::Resource::BelongsToExample.new(client, :attrs => {'id' => '123'}, :deadbeef => deadbeef) }
    ...
    it "raises an exception when initialized without a belongs_to instance" do
      lambda do
        JiraRestApi::Resource::BelongsToExample.new(client, :attrs => {'id' => '123'})
    ...
    it "can be initialized with an instance or a key value" do
      client.stub(:options => { :rest_base_path => "/foo" })
      subject = JiraRestApi::Resource::BelongsToExample.new(client, :attrs => {'id' => '123'}, :deadbeef_id => '987')
