...
describe JiraRestApi::OauthClient do

  let(:oauth_client) do
    options = { :consumer_key => 'foo', :consumer_secret => 'bar' }
    options = JiraRestApi::Client::DEFAULT_OPTIONS.merge(options)
    JiraRestApi::OauthClient.new(options)
  end
  ...
  describe "authenticating with oauth" do
    it "prepends the context path to all authorization and rest paths" do
      ...
      defaults = JiraRestApi::Client::DEFAULT_OPTIONS.merge(JiraRestApi::OauthClient::DEFAULT_OPTIONS)
      options.each do |key|
    ...
    describe "the access token" do
      ...
      it "raises an exception when accessing without initialisation" do
        ...
        }.to raise_exception(JiraRestApi::OauthClient::UninitializedAccessTokenError, 
