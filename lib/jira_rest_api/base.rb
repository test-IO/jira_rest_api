...
module JiraRestApi
  ...
  # This class provides the basic object <-> REST mapping for all JiraRestApi::Resource subclasses,
  ...
  # within the JiraRestApi API, but are always nested under the context of another
  # resource.  For example, a JiraRestApi::Resource::Comment always belongs to a
  # JiraRestApi::Resource::Issue.
  #
  ...
  class Base
    ...
    # A reference to the JiraRestApi::Client used to initialize this resource.
    attr_reader :client
    ...
    # representation of the JSON returned from the JiraRestApi API
    attr_accessor :attrs
    ...
    # These attributes will be posted to the JiraRestApi Api if save is called.
    ...
    # E.g.
    #   JiraRestApi::Resource::Issue.endpoint_name
    #     # => issue
    ...
    # E.g.
    #   JiraRestApi::Resource::Issue.collection_path
    ...
    # within the JSON returned from the JiraRestApi API.
    #
    #   class Example < JiraRestApi::Base
    #     has_one :child
    #   end
    #
    #   example = client.Example.find(1)
    #   example.child # Returns a JiraRestApi::Resource::Child
    #
    ...
    #                   relationship. E.g. <tt>has_one :child</tt> will return a <tt>JiraRestApi::Resource::Child</tt>.
    #                   Use this option to override the inferred class.
    #
    #                     has_one :child, :class => JiraRestApi::Resource::Kid
    # [:nested_under]   In some cases, the JSON return from JiraRestApi is nested deeply for particular
    ...
    def self.has_one(resource, options = {})
      ...
      child_class = options[:class] || ('JiraRestApi::Resource::' + resource.to_s.classify).constantize
    ...
    #                   of <tt>JiraRestApi::Resource::HasManyProxy</tt> containing the collection of
    #                   <tt>JiraRestApi::Resource::Child</tt>.
    #                   Use this option to override the inferred class.
    #
    #                     has_many :children, :class => JiraRestApi::Resource::Kid
    # [:nested_under]   In some cases, the JSON return from JiraRestApi is nested deeply for particular
    #                   relationships.  This option allows the nesting to be specified.
    #
    #                     # Specify a single depth of nesting.
    ...
    def self.has_many(collection, options = {})
      ...
      child_class = options[:class] || ('JiraRestApi::Resource::' + collection.to_s.classify).constantize
    ...
    # JiraRestApi::Resource::Issue returns :issue
    def to_sym
    ...
    # Fetches the attributes for the specified resource from JiraRestApi unless
    ...
    # request to JiraRestApi, depending on resource.new_record?
    #
    # Accepts an attributes hash of the values to be saved.  Will throw a
    # JiraRestApi::HTTPError if the request fails (response is not HTTP 2xx).
    def save!(attrs)
    ...
    def save(attrs)
      ...
      rescue JiraRestApi::HTTPError => exception
        set_attrs_from_response(exception.response) rescue JSON::ParserError  # Merge error status generated by JiraRestApi REST API
        save_status = false
      end
      save_status
    end

    # Sets the attributes hash from a HTTPResponse object from JiraRestApi if it is
    ...
    # Sends a delete request to the JiraRestApi Api and sets the deleted instance
